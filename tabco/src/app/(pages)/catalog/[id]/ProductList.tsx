...
export default function ProductList({ catalogId }: { catalogId: string }) {
  ...
  return (
    <div className="relative min-h-[80vh] p-4">
      ...
      <div className="sticky bottom-0 right-0 w-full border-t px-6 py-3 flex flex-wrap gap-5 justify-between items-center bg-white">
        <div className="flex font-semibold">
          <span className="mr-6">
            Total Qty: {useAppSelector(selectTotalQuantity)}
          </span>
          <span>Total Amt: ₹{useAppSelector(selectTotalAmount)}</span>
        </div>
